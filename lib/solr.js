...
/**
 * Add the simple (JSON, XML, CSV) remote resource located at the given path `options.path` into the Solr database.
 *
 * @param {Object} options -
...
Client.prototype.addSimpleResource = function(options,callback){
...
Client.prototype.addRemoteResource = function(path, options, callback){
   options = options || {};
   var fullPath = this.options.path+'/'+this.UPDATE_HANDLER + '/extract';
   ///var query = querystring.stringify(options.parameters);
 var params = {
...
//         agent: this.options.agent
   ...
   params.formData={
       commit: (options.commit === undefined ? 'false' : String(options.commit) ),
       afile:  fs.createReadStream(path),
       wt:'json'
   };
   
   for(var key in options) {
   if (key!='commit')
      params.formData['literal.'+key]=options[key];
   }
   console.log(params.formData);
   return postMultipartForm(params, callback);
}
...
function postJSON(params,callback){
  ...
  //  console.log("Solr POST ", options, params.params);
...
/**
 * HTTP POST Multipart request. Send update commands to the Solr server using form encoding (e.g. search)
 *
 * @param {Object} params
 * @param {String} params.host - IP address or host address of the Solr server
 * @param {Number|String} params.port - port of the Solr server
...
function postMultipartForm(params, callback){
  var headers = {
   ...
   */
   ...
   request.post(params, function (error, response, body) {
      console.log("Indexer response", body)
       if (error)
            return callback(error, null);
       if (response.statusCode < 200 || response.statusCode > 299){
            error = new SolrError(this, response, body);
            if (callback)  callback(error, null);
       } else {
            var data;
            try {
                  data = pickJSON(params.bigint).parse(body);
            } catch (err) {
                  error = err;
            } finally {
...
function handleJSONResponse(request, bigint, callback){
  ...
  //       console.log("Solr response ", text, request.method);
         if(response.statusCode < 200 || response.statusCode > 299){
